...
class CarlaEnvironment():
    ...
    def __init__(self, client, world, town, checkpoint_frequency=100, continuous_action=True,id=0) -> None:
...
# ----------------------------------------------------------------
    ...
    def step(self, action_idx):
        ...
        if done:
            # 当一个 episode 结束时（无论什么原因），
            # 将 fresh_start 标志设为 True。
            # 这样下一次调用 reset() 时，就会重新随机选择出生点。
            self.fresh_start = True
